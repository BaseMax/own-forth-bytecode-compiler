...
BUILTIN(25, "NOT",       _not,             0)              { push(~pop());}
...
BUILTIN(38, "QUIT", quit, 0)
{
    ...
    for (exitReq = 0; exitReq == 0;)
    {
        ...
        if (errorFlag)
            *sp = *rsp = 1;
        else if (!keyWaiting() && !(*initscript_pos) && !(*cmd_pos))
            tell("OK\n");
    }
}
...
fseek(f, 0L, SEEK_END);
sz = ftell(f);
while (getc (f) != 0) fseek (f, -2L, SEEK_CUR);
datasz = sz - ftello(f);
cmdlist = (char*)malloc(datasz);
fread(cmdlist, 1, datasz, f);
fclose(f);
